...
public class OpenDropExample4_25 extends PApplet {
    ...
    int rectHighlight = color(0, 180, 0);
    ...
    public void setup() {
        ...
        if (!connected) {
            old_transmit = true;
            for (int x = 0; x < 96; x++) {
                myPort.write((fluxels[x][frame_no]) ? 1 : 0);
            }
        } else {
            readControlData();
        }
    ...
    void drawPlane(float[][] vals, int sizeX, int sizeY, int indexY) {
        ...
        if (width < img.width) {
            imgScale = (float) (width) / img.width;
        } else {
            imgScale = 1;
        }
        //if (!(width==last_w)||!(height==last_h)||changed)  {
        background(255, 250, 240);
        image(img, width / 2 - img.width * imgScale / 2, 0, img.width * imgScale, img.height * imgScale);
        last_w = width;
        last_h = height;
        //}
        ...
        text("play", coX(37), coY(-7.5f));
        text("life", coX(37), coY(-4.5f));
        if (showFeedbackUI) {
            text("feedback", coX(37), coY(-1.5f));
        }

        text("copy last frame", coX(37), coY(5.5f));
        text("clear frame", coX(37), coY(8.5f));
        text("clear all", coX(37), coY(11.5f));

        text("save", coX(32), coY(-18.5f));
        text("load", coX(38), coY(-18.5f));
        ...
        fill(255, 0, 0);
        // reconect button
        rect(coX(-43.5f), coY(14.5f), boxSize, boxSize);
        ...
        if (showTempUI) {
            fill(255, 0, 0);
            rect(coX(-43.5f), coY(-6), boxSize * 3, boxSize);
            rect(coX(-43.5f), coY(-2), boxSize * 3, boxSize);
            rect(coX(-43.5f), coY(2), boxSize * 3, boxSize);

            fill(255, 255, 255);

            triangle(coX(-36.5f), coY(-5.3f), coX(-36.5f) + boxSize / 2, coY(-5.3f), coX(-36.5f) + boxSize / 4, coY(-5.3f) - boxSize / 4);
            triangle(coX(-36.5f), coY(-4.6f), coX(-36.5f) + boxSize / 2, coY(-4.6f), coX(-36.5f) + boxSize / 4, coY(-4.6f) + boxSize / 4);

            triangle(coX(-36.5f), coY(-1.3f), coX(-36.5f) + boxSize / 2, coY(-1.3f), coX(-36.5f) + boxSize / 4, coY(-1.3f) - boxSize / 4);
            triangle(coX(-36.5f), coY(-0.6f), coX(-36.5f) + boxSize / 2, coY(-0.6f), coX(-36.5f) + boxSize / 4, coY(-0.6f) + boxSize / 4);

            triangle(coX(-36.5f), coY(2.7f), coX(-36.5f) + boxSize / 2, coY(2.7f), coX(-36.5f) + boxSize / 4, coY(2.7f) - boxSize / 4);
            triangle(coX(-36.5f), coY(3.4f), coX(-36.5f) + boxSize / 2, coY(3.4f), coX(-36.5f) + boxSize / 4, coY(3.4f) + boxSize / 4);


            textSize(32 * imgScale);
            text("T1:", coX(-46), coY(-4.6f));
            text("T2:", coX(-46), coY(-0.6f));
            text("T3:", coX(-46), coY(3.4f));


            if (showSetTemp) {
                fill(255, 255, 0);
                text(setTemperature1 + "°C", coX(-43), coY(-4.6f));
                text(setTemperature2 + "°C", coX(-43), coY(-0.6f));
                text(setTemperature3 + "°C", coX(-43), coY(3.4f));
            } else {
                fill(255, 255, 255);
                if (Temperature1 < 150) {
                    text(nf(Temperature1, 0, 1) + "°C", coX(-43), coY(-4.6f));
                } else {
                    text("---", coX(-43), coY(-4.6));
                }
                if (Temperature1 < 150) {
                    text(nf(Temperature2, 0, 1) + "°C", coX(-43), coY(-0.6));
                } else {
                    text("---", coX(-43), coY(-0.6));
                }
                if (Temperature1 < 150) {
                    text(nf(Temperature3, 0, 1) + "°C", coX(-43), coY(3.4));
                } else {
                    text("---", coX(-43), coY(3.4));
                }
            }
        ...
        for (int i = 1; i < 18; i++) {
            ...
            circle(coX(-18 + i * 2), coY(19.5), (int) (cSize * imgScale));
        }
        fill(150, 150, 150);
    ...
    private void circle(int i, int i1, int i2) {
    }
...
// *** Play
        if (play && (millis() - time_start > interval_ms)) {
            time_start = millis();
            if (frame_no < frame_max) frame_no++;
            else frame_no = 1;
            changed = true;
        }
        ...
        if (millis() - time_edit > interval_edit_ms) showSetTemp = false;
        else showSetTemp = true;

        // ** Write to device
        if (changed && life && !transmitting) {
            transmitting = true;
            if (old_transmit) old_transmit();
            else transmit();
            transmitting = false;
        }
    ...
    void transmit() {
        ...
        for (int x = 0; x < xsize + 2; x++) //Send Fluxels plus two control lines
        {
            ...
            for (int y = 0; y < 8; y++) {
                ...
                send_value = send_value + ((fluxels[(7 - y) + x * 8][frame_no]) ? 1 : 0);
            }
        ...
        if (feedback) control_data_out[6] = 1;
        else
        ...
        if (millis() - timeout < 500) connected = true;
        ...
        for (int x = 0; x < xsize; x++) //Send Fluxels plus two control lines
        {
            ...
            // println(readdata);
            for (int y = 0; y < 8; y++) {
                fluxels_state[(7 - y) + x * 8] = (((readdata) >> (y)) & 0x01) == 0;
            }
        }
    ...
    void old_transmit() {
        {
            changed = false;
            for (int x = 0; x < 128; x++)
                myPort.write(toInt(fluxels[x][frame_no]));
        }
    ...
    public void mousePressed() {
        ...
        if ((mx > -24) && (mx < 24) && (my > -12) && (my < 12)) {
            for (int i = 0; i < electrodes_loaded; i++) {
                if (inRect(mx, my, electrodeArray[i].x, electrodeArray[i].y, electrodeArray[i].h, electrodeArray[i].v)) {
                    if ((fluxels[electrodeArray[i].e][frame_no] == false) | (cont_flag)) {
                        fluxels[electrodeArray[i].e][frame_no] = true;
                        dragg_mode = true;
                    } else {
                        fluxels[electrodeArray[i].e][frame_no] = false;
                        dragg_mode = false;
                    }
                }
            }
...
// Check frame buttons
        if (inRect(mx, my, 18, 18.5, 4, 4) & (frame_no < max_frame_no)) {
            frame_no++;
        }
        if (inRect(mx, my, -20, 18.5, 4, 4) & (frame_no > 1)) {
            frame_no--;
        }
...
// check reservoir buttons
        ...
        if (showTempUI) {
            if (inRect(mx, my, -37, -4.7, 2, 2)) {
                setTemperature1--;
                time_edit = millis();
            }
            if (inRect(mx, my, -37, -7.2, 2, 2)) {
                setTemperature1++;
                time_edit = millis();
            }

            if (inRect(mx, my, -37, -0.7, 2, 2)) {
                setTemperature2--;
                time_edit = millis();
            }
            if (inRect(mx, my, -37, -3.2, 2, 2)) {
                setTemperature2++;
                time_edit = millis();
            }

            if (inRect(mx, my, -37, 3.3, 2, 2)) {
                setTemperature3--;
                time_edit = millis();
            }
            if (inRect(mx, my, -37, 0.8, 2, 2)) {
                setTemperature3++;
                time_edit = millis();
            }
    ...
    public void mouseWheel(MouseEvent event) {
        ...
        if (inRect(mx, my, -43.5, -6, 8, 2)) {
            setTemperature1 = setTemperature1 - e;
            time_edit = millis();
        } else if (inRect(mx, my, -43.5, -2, 8, 2)) {
            setTemperature2 = setTemperature2 - e;
            time_edit = millis();
        } else if (inRect(mx, my, -43.5, 2, 8, 2)) {
            setTemperature3 = setTemperature3 - e;
            time_edit = millis();
        } else if (play) {
            interval = toInt(interval - e * 5);
            if (interval < 5) interval = 5;
            if (interval > 600) interval = 600;
            interval_ms = toInt(60000 / interval);
...
// Open Serial Port
    {
        ...
        try {
            ...
            if (i != 0) {
                ...
                for (int j = 0; j < i; ) {
                    if (Serial.list()[j].indexOf("/dev/ttyS") != 0) {
                        print("hello");
                        COMlist += toChar(10) + " " + toChar(p + 'a') + " = " + Serial.list()[j];
                        ++p;
                    }
                    ++j;
                ...
                if (p > 1) {
                    ...
                    p = toInt(COMx.toLowerCase().charAt(0) - 'a') + 1;
                }
        ...
        if (selection == null) {
            ...
            for (int i = 0; i < values.size(); i++) {
                ...
                for (int j = 0; j < name.length(); j++) {
                    if (name.charAt(j) == '1') fluxels[j * ysize + 0][i + 1] = true;
                }
                name = item.getString("y1");
                for (int j = 0; j < name.length(); j++) {
                    if (name.charAt(j) == '1') fluxels[j * ysize + 1][i + 1] = true;
                }
                name = item.getString("y2");
                for (int j = 0; j < name.length(); j++) {
                    if (name.charAt(j) == '1') fluxels[j * ysize + 2][i + 1] = true;
                }
                name = item.getString("y3");
                for (int j = 0; j < name.length(); j++) {
                    if (name.charAt(j) == '1') fluxels[j * ysize + 3][i + 1] = true;
                }
                name = item.getString("y4");
                for (int j = 0; j < name.length(); j++) {
                    if (name.charAt(j) == '1') fluxels[j * ysize + 4][i + 1] = true;
                }
                name = item.getString("y5");
                for (int j = 0; j < name.length(); j++) {
                    if (name.charAt(j) == '1') fluxels[j * ysize + 5][i + 1] = true;
                }
                name = item.getString("y6");
                for (int j = 0; j < name.length(); j++) {
                    if (name.charAt(j) == '1') fluxels[j * ysize + 6][i + 1] = true;
                }
                name = item.getString("y7");
                for (int j = 0; j < name.length(); j++) {
                    if (name.charAt(j) == '1') fluxels[j * ysize + 7][i + 1] = true;
                }
    ...
    void fillColorFlux(int x, int f) {
        ...
        if (!fluxels[x][f]) {
            if (fluxels[x][f - 1]) red = 210;
            else red = 250;
        }
        if (fluxels_state[x] & feedback) {
            red = red * 0.39 + 105;
            green = 177;
            blue = 106;
        }
    ...
    boolean inRect(double mx, double my, double x, double y, double h, double v) {
        return inRect(mx, my, x, y, (float) h, (float) v);
    }
    ...
    int fluxValue(int y, int frame) {
        ...
        for (int j = 0; j < (xsize + 2); j++) {
            val += toInt(fluxels[(xsize + 1 - j) * ysize + (y)][frame]) * pow(2, j);
        }
        ;
        return val;
    }
    ...
    class Electrode {
        ...
        void drawElectrode() {
            ...
            fill(123, 123, 123);
            if (showNumbers)
                text(e, width / 2 + (x * eSize / 2) * imgScale + (h * eSize / 5) * imgScale, img.height * imgScale / 2 - imgScale * imageShift + (y * eSize / 2) * imgScale + (v * eSize / 4) * imgScale);
        }
